...
# 消息加密密钥
EncodingAESKey = '425bKHXPIY8zqV57v3a2wY7SpCtYejfrSWxR9ZaThT3'
...
class WXBizMsgCrypt(object):
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp=None, is_json=True):
        #将公众号回复用户的消息加密打包
        ...
        if is_json:
            return ret, json.dumps(dict(Encrypt=encrypt, MsgSignature=signature, TimeStamp=timestamp, Nonce=sNonce), ensure_ascii=False)
        xmlParse = XMLParse()
        return ret,xmlParse.generate(encrypt, signature, timestamp, sNonce)

    def DecryptMsg(self, sPostData, sMsgSignature, sTimeStamp, sNonce, is_json=True):
        # 检验消息的真实性，并且获取解密后的明文
